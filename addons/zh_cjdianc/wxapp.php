...
class Zh_cjdiancModuleWxapp extends WeModuleWxapp {
	...
	public function doPageStoreList() {
		...
		for ($i = 0; $i < count($list); $i++) {
			...
			if (($time <= $ntime && $ntime < $time2) and $list[$i]['is_rest'] == '2') {
				$list[$i]['is_yy'] = 1;
			} else {
				$list[$i]['is_yy'] = 2;
			}
			$mj = pdo_getall('cjdc_reduction', array('store_id' => $list[$i]['id']));
			$time30=strtotime(date("Y-m-d",strtotime("-30 day")));
			$yorder = $userorder=pdo_fetchcolumn("select count(*)  from " . tablename("cjdc_order") ." WHERE  store_id={$list[$i]['id']} and UNIX_TIMESTAMP(time)>{$time30}  and (state in (4,5,10) || dn_state=2 || dm_state=2 || yy_state=3)");
			$list[$i]['mj'] = $mj;
			$list[$i]['score'] = $list[$i]['score']+$yorder;
		 }
		}else{
			$list = pdo_getall('cjdc_store', array('user_uid' => $_GPC['user_id'],'uniacid' => $_W['uniacid'],'state'=>2), array(), '', 'id asc');
		}
	...
	public function doPageStoreInfo() {
		...
		$sxf = pdo_get('cjdc_storetype',array('id'=>$store['md_type']));
		...
		$ordersql = "select count(id) as count from " . tablename("cjdc_order") .  "  WHERE store_id=" . $_GPC['store_id'] . " and (state in (4,5,10) || dn_state=2 || dm_state=2 || yy_state=3)";
		$order_num = pdo_fetch($ordersql);
		$zy = "select sum(money) as total from " . tablename("cjdc_order") . " WHERE store_id=" . $_GPC['store_id'] . " and (state in (4,5,10) || dn_state=2 || dm_state=2 || yy_state=3)";
		$zy = pdo_fetch($zy);
		$num = intval($order_num['count']);
		$moneys = intval($zy['total']);
		$capita = $moneys/$num;
		
		$store['store_video'] = video($store['store_video']);
		$data['storeset'] = $storeset;
		$data['store'] = $store;
		$data['store']['capita'] = round($capita);
		$data['reduction'] = $reduction;
		$data['psf'] = $psf;
		$data['storeset'] = $storeset;
		$data['sxf'] = $sxf['tg_poundage'];
		echo json_encode($data);
	...
	public function doPagePtMoney(){
		global $_W, $_GPC;
		$sql = " select * from " . tablename('cjpt_fee') . " where uniacid={$_W['uniacid']}";
	...
	public function doPageAddAddress() {
		...
		if ($res) {
			if($user['tel']==""){
			 pdo_update('cjdc_user',array('tel'=>$_GPC['tel']),array('id'=>$_GPC['user_id']));	
			}
	...
	public function doPageAddOrder() {
		...
		if($storeset['ps_mode']=='超级跑腿'){
		$data['ptps_money'] = $_GPC['ptps_money']; //超级跑腿配送费
		}else{
			$data['ptps_money'] = 0; //超级跑腿配送费
		}
		...
		if ($res) {
			if ($_GPC['coupon_id']) {
				$pd = pdo_get('cjdc_usercoupons',array('id' => $_GPC['coupon_id']));
				
				if($pd['couponnum']=='-1'){
				 pdo_update('cjdc_usercoupons', array('state'=>2,'couponnum'=>'-1'), array('id' => $_GPC['coupon_id']));
				}else if($pd['couponnum']<=1){
				 pdo_update('cjdc_usercoupons', array('state'=>1,'couponnum'=>0), array('id' => $_GPC['coupon_id']));	
				}else if($pd['couponnum']>1){
				 pdo_update('cjdc_usercoupons', array('couponnum -=' => 1), array('id' => $_GPC['coupon_id']));	
				}	
			}
			if ($_GPC['coupon_id2']) {
				$pd = pdo_get('cjdc_usercoupons',array('id' => $_GPC['coupon_id2']));
				if($pd['couponnum']<=1){
				 pdo_update('cjdc_usercoupons', array('state'=>1,'couponnum'=>0), array('id' => $_GPC['coupon_id2']));	
				}else{
				 pdo_update('cjdc_usercoupons', array('couponnum -=' => 1), array('id' => $_GPC['coupon_id2']));	
				}			
			}
...
//余额支付
				if ($_GPC['money'] > 0) {
					...
					$this->getapi("" . $_W['siteroot'] . "app/index.php?i=" . $_W['uniacid'] . "&c=entry&a=wxapp&do=JsCommission&m=zh_cjdianc&order_id=" . $order_id);
...
//储值卡支付
				if ($_GPC['money'] > 0) {
					$user = pdo_get('cjdc_storevip', array('user_id' => $_GPC['user_id'],'store_id'=>$_GPC['store_id']));
					if ($user['money'] < $_GPC['money']) {
						echo json_encode('储值卡余额不足');die;
					}

					pdo_update('cjdc_storevip', array('money -=' => $_GPC['money']), array('user_id' => $_GPC['user_id'],'store_id'=>$_GPC['store_id']));
					$this->getapi("" . $_W['siteroot'] . "app/index.php?i=" . $_W['uniacid'] . "&c=entry&a=wxapp&do=JsCommission&m=zh_cjdianc&order_id=" . $order_id);
					$data4['money'] = $_GPC['money'];
					$data4['user_id'] = $_GPC['user_id'];
					$data4['type'] = 2;
	...
	public function doPageAddDnOrder() {
		...
		if ($table['status'] == 0) {
			...
			$data['code'] = time() . rand(1000, 9999) . $_GPC['user_id']; //code
			$data['dn_state'] = 1;
			if ($_GPC['pay_type'] == 2 || $_GPC['pay_type'] == 5) {
				$data['dn_state'] = 2;
			}
			$data['form_id'] = $_GPC['form_id']; //下单成功通知
			...
			if ($_GPC['coupon_id2']) {
				...
				if ($_GPC['pay_type'] == 2) {
					if ($_GPC['money'] > 0) {
						...
						pdo_update('cjdc_user', array('wallet -=' => $_GPC['money']), array('id' => $_GPC['user_id']));
						$cctv=$this->getapi("" . $_W['siteroot'] . "app/index.php?i=" . $_W['uniacid'] . "&c=entry&a=wxapp&do=JsCommission&m=zh_cjdianc&order_id=" . $order_id);
						$data4['money'] = $_GPC['money'];
						$data4['user_id'] = $_GPC['user_id'];
						$data4['type'] = 2;
						$data4['note'] = '店内订单';
...
//余额支付
	...
	public function doPageDmOrder() {
		...
		$data['yhq_money'] = $_GPC['yhq_money'];
		$data['coupon_id2'] = $_GPC['coupon_id2'];
		$data['yhq_money2'] = $_GPC['yhq_money2'];
		$data['discount'] = $_GPC['yhq_money'] + $_GPC['yhq_money2'];
		$data['code'] = time() . rand(1000, 9999) . $_GPC['user_id']; //code
		$res = pdo_insert('cjdc_order', $data);
		$order_id = pdo_insertid();
		if ($res) {
...
//储值卡支付
	...
	public function doPagePay() {
		...
		if ($_GPC['type'] == 2) {
			...
			$order = pdo_get('cjdc_dxorder', array('id' => $_GPC['order_id']));
		} else {
			$order = pdo_get('cjdc_order', array('id' => $_GPC['order_id']));
		}
		$appid = $res2['appid'];
...
//店内
	...
	public function doPageOkOrder() {
		...
		$order = pdo_get('cjdc_order',array('id'=>$_GPC['order_id']));
		if ($res) {
			pdo_query("UPDATE ".tablename('cjpt_dispatch')." SET state = :state, wc_time = :wc_time WHERE order_id = :order_id", array(':state' => 4, ':wc_time' => time(), ':order_id' => $order['order_num']));
	...
	public function doPageStoreType() {
		global $_W, $_GPC;
		$res = pdo_getall('cjdc_storetype', array('uniacid' => $_W['uniacid']), array(), '', 'id asc');
		echo json_encode($res);
	}
	...
	public function doPageUserMadd(){
		global $_W, $_GPC;
		if($_GPC['messages']==2){
		  $rese = pdo_getall('cjdc_userm', array('user_id'=>$_GPC['user_id'],'message'=>1));
		  $ress = pdo_getall('cjdc_userm', array('user_id'=>$_GPC['user_id'],'message'=>2));
		  if(empty($rese)){
			 pdo_insert('cjdc_userm',array('user_id'=>$_GPC['user_id'],'message'=>1));	 	
		   }
		   if(empty($ress)){
			 pdo_insert('cjdc_userm',array('user_id'=>$_GPC['user_id'],'message'=>2));	 	
		   }
		}else{
			$res = pdo_get('cjdc_userm', array('user_id'=>$_GPC['user_id'],'message'=>$_GPC['message']));
			if(empty($res)){
			 pdo_insert('cjdc_userm',array('user_id'=>$_GPC['user_id'],'message'=>$_GPC['message']));	 	
		   }
		}
	}
	
	public function doPageUserM(){
		global $_W, $_GPC;
		if($_GPC['state']==2){
		$res = pdo_update('cjdc_userm', array('state'=>0),array('user_id'=>$_GPC['user_id'],'message'=>1));
		$res = pdo_update('cjdc_userm', array('state'=>0),array('user_id'=>$_GPC['user_id'],'message'=>2));		
		}
		
		if($_GPC['state']==1){
		 $res = pdo_update('cjdc_userm', array('state'=>0),array('user_id'=>$_GPC['user_id'],'message'=>$_GPC['message']));
		}
		if($_GPC['messages']==2){
		 $res = pdo_getall('cjdc_userm', array('user_id'=>$_GPC['user_id'],'message <='=>2));
		 echo json_encode($res);
		}else{
		 $res = pdo_get('cjdc_userm', array('user_id'=>$_GPC['user_id'],'message'=>$_GPC['message']));
		 echo json_encode($res);	
	...
	public function doPageMessage() {
		...
		function set_msg($_W) {
            ...
            'data'=>[
                'number1'=>array('value'=>$res2['order_num']), 
                'name2'=>array('value'=>$res2['name']), 
                'phone_number11'=>array('value'=>$res2['tel']), 
                'amount10'=>array('value'=>$res2['money']), 
				'date3'=>array('value'=>$res2['pay_time']), 
					]
				];
	...
	//普通领劵中心优惠券列表
	public function doPagePtCoupons() {
		global $_W, $_GPC;
		$system = pdo_get('cjdc_system', array('uniacid' => $_W['uniacid']));
		if ($_GPC['lat']) {
			$lat = $_GPC['lat'];
		...
		$sql = "select xx.* from (SELECT distinct a.*, ROUND(6378.138*2*ASIN(SQRT(POW(SIN(($lat*3.1415926/180-SUBSTRING_INDEX(coordinates, ',', 1)*3.1415926/180)/2),2)+COS($lat*3.1415926/180)*COS(SUBSTRING_INDEX(coordinates, ',', 1)*3.1415926/180)*POW(SIN(($lng*3.1415926/180-SUBSTRING_INDEX(coordinates, ',', -1)*3.1415926/180)/2),2)))*1000) AS juli,b.name as store_name,b.logo,b.coordinates,c.ps_jl FROM " . tablename("cjdc_coupons"). "a" . " left join " . tablename("cjdc_storeset"). "c on c.store_id=a.store_id left join" . tablename("cjdc_store") . "b on b.id=a.store_id where a.category=1 and a.store_id>0 and a.stock>0 and a.uniacid={$_W['uniacid']} and UNIX_TIMESTAMP(a.start_time)<={$time} and  UNIX_TIMESTAMP(a.end_time)>={$time} order by a.id DESC) xx where xx.juli<=" . $system['distance'];
		
		//$sql = "select a.*,b.name as store_name,b.coordinates,c.ps_jl from" . tablename("cjdc_coupons"). "a" . " left join " . tablename("cjdc_storeset"). "c on c.store_id=a.store_id left join" . tablename("cjdc_store") . "b on b.id=a.store_id where a.category=1 and a.store_id>0 and a.stock>0 and a.uniacid={$_W['uniacid']} and UNIX_TIMESTAMP(a.start_time)<={$time} and  UNIX_TIMESTAMP(a.end_time)>={$time} order by a.id DESC";
		...
		for ($i = 0; $i < count($coupons); $i++) {
			...
			for ($k = 0; $k < count($usercoupons); $k++) {
				if ($coupons[$i]['id'] == $usercoupons[$k]['coupon_id']) {
					$lqcc = pdo_get('cjdc_usercoupons',array('user_id' => $_GPC['user_id'],'coupon_id'=>$coupons[$i]['id'],'state'=>2));
					$qlnum = pdo_getall('cjdc_usercoupons',array('user_id' => $_GPC['user_id'],'coupon_id'=>$coupons[$i]['id'],'state'=>1));
					if($lqcc){
					$coupons[$i]['state'] = 1; //领取	
					}else if($coupons[$i]['is_lqs']==2&&$coupons[$i]['dc_num']>count($qlnum)){
					$coupons[$i]['state'] = 2;
					}else{
						$coupons[$i]['state'] = 1;
					}
	...
	//异业领劵中心优惠券列表
	public function doPageYyCoupons() {
		global $_W, $_GPC;
		$system = pdo_get('cjdc_system', array('uniacid' => $_W['uniacid']));
		if ($_GPC['lat']) {
			$lat = $_GPC['lat'];
		...
		$time = strtotime(date("Y-m-d"));
		$sql = "select xx.* from (SELECT distinct a.*, ROUND(6378.138*2*ASIN(SQRT(POW(SIN(($lat*3.1415926/180-SUBSTRING_INDEX(coordinates, ',', 1)*3.1415926/180)/2),2)+COS($lat*3.1415926/180)*COS(SUBSTRING_INDEX(coordinates, ',', 1)*3.1415926/180)*POW(SIN(($lng*3.1415926/180-SUBSTRING_INDEX(coordinates, ',', -1)*3.1415926/180)/2),2)))*1000) AS juli,b.name as store_name,b.logo,b.coordinates,c.ps_jl FROM " . tablename("cjdc_coupons"). "a" . " left join " . tablename("cjdc_storeset"). "c on c.store_id=a.store_id left join" . tablename("cjdc_store") . "b on b.id=a.store_id where a.category=2 and a.is_dj=0 and a.store_id>0 and a.stock>0 and a.uniacid={$_W['uniacid']} and UNIX_TIMESTAMP(a.start_time)<={$time} and  UNIX_TIMESTAMP(a.end_time)>={$time} order by a.id DESC) xx where xx.juli<=" . $system['distance'];
		$coupons = pdo_fetchall($sql);
		//$coupons=pdo_getall('cjdc_coupons',array('uniacid'=>$_W['uniacid'],'store_id'=>$_GPC['store_id']));
		$type = pdo_getall('cjdc_storetype', array('uniacid' => $_W['uniacid']));
		$usercoupons = pdo_getall('cjdc_usercoupons', array('user_id' => $_GPC['user_id'],'fxtime'=>0));
		for ($i = 0; $i < count($coupons); $i++) {
			$coupons[$i]['state'] = 2;
			for ($j = 0; $j < count($type); $j++) {
				if (strpos($coupons[$i]['type_id'], ',')) {
	...
	public function doPageMyyyCoupons(){
		global $_W, $_GPC;
		$time = strtotime(date("Y-m-d"));
		$sql = "select a.couponnum,a.id as ucid,b.id,b.name as coupon_name,b.store_id,b.type_id,b.full,b.commission,b.commission_td,b.type as coupon_type,b.start_time,b.end_time,b.reduce,b.instruction,c.name as store_name,c.logo from " . tablename("cjdc_usercoupons") . "a" . " left join " .tablename('cjdc_coupons'). "b on b.id=a.coupon_id left join" . tablename("cjdc_store") . "c on c.id = b.store_id where b.category=2 and a.fxtime=0 and ((a.couponnum>0) or (a.couponnum=-1)) and a.state=2 and a.user_id={$_GPC['user_id']} and a.uniacid={$_W['uniacid']} and UNIX_TIMESTAMP(b.start_time)<={$time} and  UNIX_TIMESTAMP(b.end_time)>={$time} order by a.id DESC";
		$coupons = pdo_fetchall($sql);
		echo json_encode($coupons);	
	}
	
	public function doPageMyyCoupons(){
	  global $_W, $_GPC;
		$time = strtotime(date("Y-m-d"));
		if($_GPC['storeid']>0){
		$sql = "select a.couponnum,a.id as ucid,b.id,b.name as coupon_name,b.store_id,b.type_id,b.full,b.commission,b.commission_td,b.type as coupon_type,b.start_time,b.end_time,b.reduce,b.instruction,c.name as store_name,c.logo from " . tablename("cjdc_usercoupons") . "a" . " left join " .tablename('cjdc_coupons'). "b on b.id=a.coupon_id left join" . tablename("cjdc_store") . "c on c.id = b.store_id where b.category=2 and a.fxtime=0 and ((a.couponnum>0) or (a.couponnum=-1)) and a.state=2 and b.is_dj=1 and b.store_id={$_GPC['storeid']} and a.user_id={$_GPC['user_id']} and a.uniacid={$_W['uniacid']} and UNIX_TIMESTAMP(b.start_time)<={$time} and  UNIX_TIMESTAMP(b.end_time)>={$time} order by a.id DESC";
		}else{
		$sql = "select a.couponnum,a.id as ucid,b.id,b.name as coupon_name,b.store_id,b.type_id,b.full,b.commission,b.commission_td,b.type as coupon_type,b.start_time,b.end_time,b.reduce,b.instruction,c.name as store_name,c.logo from " . tablename("cjdc_usercoupons") . "a" . " left join " .tablename('cjdc_coupons'). "b on b.id=a.coupon_id left join" . tablename("cjdc_store") . "c on c.id = b.store_id where b.category=2 and a.fxtime=0 and ((a.couponnum>0) or (a.couponnum=-1)) and a.state=2 and b.is_dj=0 and a.user_id={$_GPC['user_id']} and a.uniacid={$_W['uniacid']} and UNIX_TIMESTAMP(b.start_time)<={$time} and  UNIX_TIMESTAMP(b.end_time)>={$time} order by a.id DESC";
		}
		$coupons = pdo_fetchall($sql);
		$type = pdo_getall('cjdc_storetype', array('uniacid' => $_W['uniacid']));
		$usercoupons = pdo_getall('cjdc_usercoupons', array('user_id' => $_GPC['uid'],'uid'=>$_GPC['user_id'],'fxtime'=>0));
		for ($i = 0; $i < count($coupons); $i++) {
			$coupons[$i]['state'] = 2;
			for ($j = 0; $j < count($type); $j++) {
				if (strpos($coupons[$i]['type_id'], ',')) {
	...
	public function doPageCoupons() {
		global $_W, $_GPC;
		$time = strtotime(date("Y-m-d"));
		$sql = "select  * from " . tablename("cjdc_coupons") . " where category=1 and uniacid={$_W['uniacid']} and store_id={$_GPC['store_id']} and UNIX_TIMESTAMP(start_time)<={$time} and  UNIX_TIMESTAMP(end_time)>={$time} order by id DESC";
		$coupons = pdo_fetchall($sql);
		//$coupons=pdo_getall('cjdc_coupons',array('uniacid'=>$_W['uniacid'],'store_id'=>$_GPC['store_id']));
		$type = pdo_getall('cjdc_storetype', array('uniacid' => $_W['uniacid']));
		$usercoupons = pdo_getall('cjdc_usercoupons', array('user_id' => $_GPC['user_id']));
	...
	//入驻优惠券
	public function doPageRzCoupon() {
		global $_W, $_GPC;
		$time = strtotime(date("Y-m-d"));
		if ($_GPC['store_id']) {
			$store = pdo_get('cjdc_store', array('id' => $_GPC['store_id']));
			if($store['is_czhd']==1){
			 $where = " and (b.store_id=" . $_GPC['store_id'] . " || b.store_id=0 || b.is_hd=1) and  (find_in_set('{$store['md_type']}',type_id) || type_id=0)";
			}else{
			 $where = " and (b.store_id=" . $_GPC['store_id'] . " || b.store_id=0) and  (find_in_set('{$store['md_type']}',type_id) || type_id=0)";
			}
			
		}
		$sql = "select a.*,b.full,b.reduce,b.instruction,b.store_id,b.name,b.category,b.commission,b.commission_td,b.type_id,b.type as coupon_type,b.end_time,d.name as store_name,d.logo from " . tablename("cjdc_usercoupons") . " a" . " left join " . tablename("cjdc_coupons") . " b on b.id=a.coupon_id " . "  left join " . tablename("cjdc_store") . " d on d.id=b.store_id  WHERE  UNIX_TIMESTAMP(b.start_time)<={$time} and  UNIX_TIMESTAMP(b.end_time)>={$time} and a.state=2 and  a.user_id={$_GPC['user_id']} and b.is_hy=1" . $where;
		$res = pdo_fetchall($sql);
		$type = pdo_getall('cjdc_storetype', array('uniacid' => $_W['uniacid']));
		for ($i = 0; $i < count($res); $i++) {
			$data = array();
	...
	public function doPageMyCoupons() {
		...
		$sql = "select a.*,b.full,b.reduce,b.instruction,b.store_id,b.name,b.category,b.commission,b.commission_td,b.type_id,b.type as coupon_type,b.end_time,d.name as store_name,d.logo from " . tablename("cjdc_usercoupons") . " a" . " left join " . tablename("cjdc_coupons") . " b on b.id=a.coupon_id " . "  left join " . tablename("cjdc_store") . " d on d.id=b.store_id  WHERE  UNIX_TIMESTAMP(b.start_time)<={$time} and  UNIX_TIMESTAMP(b.end_time)>={$time} and a.state=2 and  a.user_id={$_GPC['user_id']} and b.is_hy=2" . $where;
		$res = pdo_fetchall($sql);
		$type = pdo_getall('cjdc_storetype', array('uniacid' => $_W['uniacid']));
		for ($i = 0; $i < count($res); $i++) {
			$data = array();
	...
	public function doPageQtPrint() {
		...
		foreach ($res3 as $key => $value) {
			...
			if ($res['type'] == 4) {
				...
				$content .= "<B>" . "  订单编号  #" . $res['oid'] . "</B>" . $style . $style;
				if ($res['type'] == 1) {
				...
				if ($res['type'] == 3) {
					...
					$content .= "<B>" . "联系电话：" . $res['tel'] . "</B>" . $style;
					$content .= "<B>" . "联系人：" . $res['name'] . "</B>" . $style;
					$content .= "--------------------------------" . $style;
				}
				if ($res2) {
...
//退款
	...
	public function doPageFxPay() {
		...
		$root = MODULE_URL . 'payment/wechat/notify.php';
		$store = pdo_get('cjdc_retail', array('id' => $_GPC['rz_id']));
	...
	//分享异业页面
	public function doPageSaveYys(){
		global $_W, $_GPC;
		$data['user_id'] = $_GPC['user_id'];
		$data['time'] = time();
		pdo_insert('cjdc_yytime', $data);
		$res = pdo_insertid();
		echo json_encode($res);
	}
	
	//提交异业身份
	public function doPageYyTime() {
		global $_W, $_GPC;
		$res = pdo_get('cjdc_yytime',array('id'=>$_GPC['id']));
		echo json_encode($res);
	}
	
	public function doPageSaveYyr() {
		global $_W, $_GPC;
		$data1['state'] = 2;
		$ss = pdo_get('cjdc_yytime',array('id'=>$_GPC['id']));
		$store = pdo_get('cjdc_store',array('admin_id'=>$ss['user_id']));   
		$user = pdo_get('cjdc_user',array('id'=>$_GPC['user_id'])); 
		
		$data['tel'] = $_GPC['phone'];
		$data['juese'] = 1;
		$data['yy_time'] = date("Y-m-d", time());
		if($user['yy_id']==0){
		$data['yy_id'] = $store['id'];	
		}else{
		$data['yy_id'] = $user['yy_id'].','.$store['id'];	
		}
		if(strpos($user['yy_id'],$store['id']) !== false){
			echo '2';  	
		}else{
			 pdo_update('cjdc_yytime', $data1, array('id' => $_GPC['id']));
	...
	public function doPageSaveRzsq() {
		...
		//$uid = pdo_get('cjdc_fxuser', array('fx_user' => $_GPC['user_id']), array('user_id'));
		//$data['user_uid'] = $uid['user_id'];
		
		$data['name'] = $_GPC['name'];
		if($_GPC['admin_id']>0){
		$data['sq_id'] = $_GPC['user_id'];
		$data['user_id'] = $_GPC['admin_id'];
		$data['admin_id'] = $_GPC['admin_id'];
		$data['user_uid'] = $_GPC['user_id'];		
		}else{
		$data['sq_id'] = $_GPC['user_id'];
		$data['user_id'] = $_GPC['user_id'];
		...
		if ($_GPC['id'] == ''||$_GPC['id'] == 0) {
			...
			$rz_id = pdo_insertid();
			if($res&&$_GPC['cid']>0){
				pdo_update('cjdc_usercoupons',array('state'=>1),array('id'=>$_GPC['cid']));
			}
	...
	//推广员店铺计算
	public function doPageStoreNum(){
		global $_W, $_GPC;
		$count = pdo_get('cjdc_store', array('uniacid' => $_W['uniacid'], 'user_uid' => $_GPC['user_id']), array('count(id) as count'));
...
//喜讯
	...
	public function doPageJjOrder() {
		...
		if (($type['pay_type'] == 1 || $type['pay_type'] == 2) and $type['money'] > 0) {
			...
			if ($type['pay_type'] == 2) {
//余额退款
				$rst = pdo_get('cjdc_qbmx', array('user_id' => $type['user_id'], 'order_id' => $type['id'],'type'=>1));
				if (!$rst) {
					$tk['money'] = $type['money'];
					$tk['order_id'] = $type['id'];
...
//退款成功
				...
				function set_msg($_W) {
					...
					$res = pdo_get('cjdc_message', array('uniacid' => $_W['uniacid']));
					$res2 = pdo_get('cjdc_order', array('id' => $_GET['order_id']));
					$res3 = pdo_get('cjdc_order_goods',array('order_id'=>$_GET['order_id']));
					$user = pdo_get('cjdc_user', array('id' => $res2['user_id']));
					$store = pdo_get('cjdc_store', array('id' => $res2['store_id']));
					$formwork =[
					'touser'=>$user['openid'],
					...
					'data'=>[
						...
						'date3'=>array('value'=>date("Y-m-d H:i:s")), 
						'thing4'=>array('value'=>$res3['name']. ' ' .$res3['spec']), 
						'thing5'=>array('value'=>'非常抱歉,商家暂时无法接单哦退款将尽快送达您的账户,请耐心等待'), 
							]
						];
					$url = "https://api.weixin.qq.com/cgi-bin/message/subscribe/send?access_token=" . $access_token . "";
					$formworks = json_encode($formwork);
...
//退款成功
	...
	public function doPageStoreStatistics() {
		...
		$zy = "select sum(money) as total from " . tablename("cjdc_order") . " WHERE store_id=" . $_GPC['store_id'] . " and state in (0,2,3,4,5,10) and yy_state!=1";
		$zy = pdo_fetch($zy);
		$zynum = "select count(id) as count from " . tablename("cjdc_order") . " WHERE store_id=" . $_GPC['store_id'] . " and state in (0,2,3,4,5,10) and yy_state!=1";
		$zynum = pdo_fetch($zynum);
		$zy = empty($zy['total']) ? '0.00' : $zy['total'];
		
		
		$data['zy'] = $zy;
		$data['zynum'] = $zynum['count'];
	...
	public function doPageGetFxData() {
		global $_W, $_GPC;
		$time = time();
		$sql2 = "select sum( case when state=1 or end_time>{$time} then money else 0 end) as djyj, sum( case when state=2 and end_time<{$time} then money else 0 end) as yxjy from  " . tablename('cjdc_earnings') . " where user_id={$_GPC['user_id']}";
		$yj = pdo_fetch($sql2);
	...
	public function doPageMyyCode() {
		...
		function getCoade($user_id) {
			...
			function set_msg($user_id) {
				$access_token = getaccess_token();
				$data2 = array("scene" => $user_id,
					"page" => "zh_cjdianc/pages/quan/zijiyiye/zijiyiye",
					"width" => 400);
				$data2 = json_encode($data2);
				$url = "https://api.weixin.qq.com/wxa/getwxacodeunlimit?access_token=" . $access_token . "";
				$ch = curl_init();
		...
		if (preg_match('/^(data:\s*image\/(\w+);base64,)/', $base64_image_content, $result)) {
			$type = $result[2];
			$new_file = IA_ROOT . "/addons/zh_cjdianc/fximg/";
			if (!file_exists($new_file)) {
				//检查是否有该文件夹，如果没有就创建，并给予最高权限
				mkdir($new_file, 0777);
			}
		...
		echo $_W['siteroot'] . "addons/zh_cjdianc/fximg/" . $wname;
	...
	public function doPageBinding() {
		...
		//$user = pdo_get('cjdc_user', array('id' => $_GPC['fx_user']));
		if ($set['is_open'] == 1) {
			if ($_GPC['user_id'] == $_GPC['fx_user']) {
				...
				$res3 = pdo_insert('cjdc_fxuser', array('user_id' => 0, 'fx_user' => $_GPC['fx_user'], 'time' => date('Y-m-d H:i:s', time())));
						//pdo_insert('cjdc_retail',array('user_id'=>$_GPC['fx_user'],'time'=>time(),'sh_time'=>time(),'uniacid'=>$_W['uniacid'],'user_name'=>$user['name'],'state'=>2));
				if ($res3) {
					echo '1';
				} else {
					echo '2';
	...
	public function doPageMyTeams(){
		global $_W, $_GPC;
		$store_id = $_GPC['store_id'];
		$res = pdo_getall('cjdc_user',array('uniacid' => $_W['uniacid'],'yy_id'=>$store_id));
		for($i=0;$i<count($res);$i++){
		  $ordersql = "select count(a.id) as count from " . tablename("cjdc_order") . " a " . " left join " . tablename("cjdc_usercoupons") . " b on b.id=a.coupon_id   WHERE a.store_id=" . $store_id . " and b.uid=" . $res[$i]['id'] . " and (a.state in (4,5,10) || a.dn_state=2 || a.dm_state=2 || a.yy_state=3)";
		  $order = pdo_fetch($ordersql);
		  $res[$i]['ordernum']=$order['count'];
		}
		echo json_encode($res);
	}
	...
	public function doPageMyCommission() {
		...
		$lei = "select sum(money) as tx_cost from " . tablename("cjdc_earnings") . " WHERE  state=2 and end_time<{$time} and user_id=" . $_GPC['user_id'];
		$lei = pdo_fetch($lei);
	...
	public function doPageCommissionList() {
		...
		$times = time();
		if($_GPC['type']==1){
		$sql = "select a.id,a.order_id,a.state,a.money,a.note,a.time,a.is_shop,a.end_time,b.order_num,b.type,c.ordersn,e.name,e.img from ". tablename("cjdc_earnings") . "a left join" . tablename('ewei_shop_order') . "c on a.order_id=c.id left join" . tablename('cjdc_user'). " e on e.id = a.son_id left join" . tablename('cjdc_order') . "b on a.order_id=b.id where a.user_id={$_GPC['user_id']} and a.state={$_GPC['type']} or a.end_time > {$times} order by a.id  desc ";
		}else{
		$sql = "select a.id,a.order_id,a.state,a.money,a.note,a.time,a.is_shop,a.end_time,b.order_num,b.type,c.ordersn,e.name,e.img from ". tablename("cjdc_earnings") . "a left join" . tablename('ewei_shop_order') . "c on a.order_id=c.id left join" . tablename('cjdc_user'). " e on e.id = a.son_id left join" . tablename('cjdc_order') . "b on a.order_id=b.id where a.user_id={$_GPC['user_id']} and a.state={$_GPC['type']} and a.end_time < {$times} order by a.id  desc ";
		}
		$select_sql = $sql . " LIMIT " . ($pageindex - 1) * $pagesize . "," . $pagesize;
		$lists = pdo_fetchall($select_sql, $data);
		if($lists){
	...
	//佣金计算
	public function doPageJsCommission($order_id) {
		global $_W, $_GPC;
		//订单类型
		$order_id = $_GPC['order_id'];
		$order = pdo_get('cjdc_order', array('id' => $order_id), array('type', 'money', 'user_id','store_id','coupon_id','coupon_id2','fwy_id'));
		$commission = pdo_get('cjdc_fxset', array('uniacid' => $_W['uniacid']));
		$store = pdo_get('cjdc_store', array('id' => $order['store_id']));
		$fwy_id = $order['fwy_id'];
		$staff_money = $store['staff_money'];
		if($order['coupon_id']>0){
			$commissionid=$order['coupon_id'];
			}
			
		if($order['coupon_id2']>0){
			$commissionid=$order['coupon_id2'];
			}
			
		
		$coupon = pdo_get('cjdc_usercoupons',array('id'=>$commissionid));
		//获取佣金比例
		if ($commission['is_open'] == 1) {
			...
			if (($commission['type'] == 1 && $order['type'] != 1) or ($commission['type'] == 2 && $order['type'] == 1) or ($commission['type'] == 3)) {
				...
				$tgyuser = pdo_get('cjdc_user',array('id'=>$user['user_id']));
				$isyyuser = pdo_get('cjdc_store',array('admin_id'=>$user['user_id']));
				$isstore = pdo_get('cjdc_store',array('admin_id'=>$coupon['uid']));
				
				if($order['coupon_id2']==0&&$order['coupon_id']==0&&$user['user_id']==0){
					pdo_update('cjdc_fxuser',array('user_id'=>$store['admin_id']),array('fx_user'=>$order['user_id']));
				}
		
				//服务员提成计算
				  if($fwy_id>0){
					$fwyid = pdo_get('cjdc_yg',array('id'=>$fwy_id));
					$money = $order['money'] * ($staff_money / 100); //佣金
					$data6['user_id'] = $fwyid['user_id']; //上线id
					$data6['son_id'] = $order['user_id']; //下线id
					$data6['money'] = $money; //金额
					$data6['time'] = time(); //时间
					$data6['order_id'] = $order_id;
					$data6['state'] = $state;
					$data6['note'] = '服务订单提成';
					$data6['uniacid'] = $_W['uniacid'];
					
					$yyj = pdo_insert('cjdc_earnings',$data6); 
					pdo_update('cjdc_order',array('fwycommission'=>$money),array('id'=>$order_id));
				  }
				  ...
				  if($commissionid>0){
					 if($coupon['uid']>0){
						...
						$sonid = $coupon['user_id']; 
					 }
					 
			         $couponmoney = pdo_get('cjdc_coupons',array('id'=>$coupon['coupon_id']));
					 
					 if($couponmoney['commission']>0){
						 $coupons['user_id'] = $sonid;
						 $coupons['son_id'] = $order['user_id'];
						 $coupons['money'] = $couponmoney['commission'];
						 $coupons['time'] = time();
						 $coupons['order_id'] = $order_id;
						 $coupons['state'] = $state;
						 $coupons['note'] = '优惠券佣金';
						 $coupons['uniacid'] = $_W['uniacid'];
						 pdo_insert('cjdc_earnings', $coupons); 
						 pdo_update('cjdc_order',array('commission'=>$couponmoney['commission']),array('id'=>$order_id));
					 
					 }else if($couponmoney['commission_td']>0){
						 $money = $order['money'] * ($couponmoney['commission_td'] / 100);
						 $coupons['user_id'] = $sonid;
						 $coupons['son_id'] = $order['user_id'];
						 $coupons['money'] = $money;
						 $coupons['time'] = time();
						 ...
						 $coupons['uniacid'] = $_W['uniacid'];
						 pdo_insert('cjdc_earnings', $coupons); 
						 pdo_update('cjdc_order',array('commission'=>$money),array('id'=>$order_id));
					 
					 }
					 
					 if($isstore&&$user['user_id']==0&&$couponmoney==2){
						 pdo_update('cjdc_fxuser',array('user_id'=>$coupon['uid']),array('fx_user'=>$order['user_id']));
				...
				//推广员佣金计算
				if($store['user_uid']>0){
					 $rgcommission = pdo_get('cjdc_storetype', array('id' => $store['md_type']));
					 
						 if ($order['type'] == 1) {
							$tgmoney = $order['money'] * ($rgcommission['tg_poundage'] / 100); //推广者外卖佣金
						}else if($order['type'] == 2){
							$tgmoney = $order['money'] * ($rgcommission['tg_dnpoundage'] / 100); //推广者店内佣金
						}else if($order['type'] == 4){
							$tgmoney = $order['money'] * ($rgcommission['tg_dmpoundage'] / 100); //推广者当面佣金
						}else if($order['type'] == 3){
							$tgmoney = $order['money'] * ($rgcommission['tg_ydpoundage'] / 100); //推广者预约佣金
						}
					if($rgcommission['tg_poundage']>0){
						 $tgyj['user_id'] = $store['user_uid'];
						 $tgyj['son_id'] = $order['user_id'];
						 $tgyj['money'] = $tgmoney;
						 $tgyj['time'] = time();
						 $tgyj['order_id'] = $order_id;
						 $tgyj['state'] = $state;
						 $tgyj['note'] = '下级店铺订单佣金';
						 $tgyj['uniacid'] = $_W['uniacid'];
						 pdo_insert('cjdc_earnings', $tgyj);
						}						 
					}
				 ...
				 if ($user['user_id']>0) {
				  if($tgyuser['tgy']==1){
					$money = $order['money'] * ($store['coupon_money'] / 100); //一级佣金
					...
					$data6['note'] = '订单佣金';
					$data6['uniacid'] = $_W['uniacid'];
					pdo_update('cjdc_order',array('yycommission'=>$money),array('id'=>$order_id));
					$yyj = pdo_insert('cjdc_earnings',$data6);
					}else if($isyyuser){
					$money = $order['money'] * ($store['coupon_money'] / 100); //佣金
					$data6['user_id'] = $user['user_id']; //上线id
					$data6['son_id'] = $order['user_id']; //下线id
					$data6['money'] = $money; //金额
					$data6['time'] = time(); //时间
					$data6['order_id'] = $order_id;
	...
	public function doPageVipMx(){
		...
		$sql = "select a.* ,b.order_num,b.pay_type,c.name,c.logo from " . tablename("cjdc_qbmx") . "a" . " left join " . tablename("cjdc_order") . " b on b.id=a.order_id left join" . tablename('cjdc_store') . "c on c.id=b.store_id where a.user_id={$_GPC['user_id']} and b.store_id={$_GPC['store_id']} and b.pay_type=5" . $where . " order by a.id DESC";
		$res = pdo_fetchall($sql);
		echo json_encode($res);
	}
	...
	public function doPageQbmx() {
		...
		$sql = "select a.* ,b.order_num,b.pay_type,c.name,c.logo from " . tablename("cjdc_qbmx") . " a" . " left join " . tablename("cjdc_order") . " b on b.id=a.order_id left join" . tablename('cjdc_store') . "c on c.id=b.store_id where a.user_id={$_GPC['user_id']}" . $where . " order by a.id DESC";
		$res = pdo_fetchall($sql);
		//$res = pdo_getall('cjdc_qbmx', array('user_id' => $_GPC['user_id']), array(), '', 'id DESC');
		echo json_encode($res);
	}
	
	public function doPageStoreNote(){
		global $_W, $_GPC;
		$res = pdo_get('cjdc_store', array('uniacid' => $_W['uniacid'],'id'=>$_GPC['store_id']));
		echo json_encode($res);
	}
	
	//充值活动
	public function doPageCzdx() {
		global $_W, $_GPC;
		$res = pdo_getall('cjdc_storenote', array('uniacid' => $_W['uniacid']), array(), '', 'money asc');
		echo json_encode($res);
	}
	...
	//商家充值活动
	public function doPageStoreCzhd() {
		global $_W, $_GPC;
		$res = pdo_getall('cjdc_cjhd', array('uniacid' => $_W['uniacid'],'store_id'=>$_GPC['store_id'],'state'=>1), array(), '', 'id asc');
		echo json_encode($res);
	}
	
	public function doPageUserMoney() {
		global $_W, $_GPC;
		$res = pdo_get('cjdc_storevip', array('uniacid' => $_W['uniacid'],'store_id'=>$_GPC['store_id'],'user_id'=>$_GPC['user_id']));
		echo json_encode($res);
	}
	
	public function doPageStoreMoney() {
		global $_W, $_GPC;
		$sql = "select a.* ,b.name,b.logo from " . tablename("cjdc_storevip") . " a" . " left join " . tablename("cjdc_store") . " b on b.id=a.store_id   where a.user_id={$_GPC['user_id']} ";
		$res = pdo_fetchall($sql);
		echo json_encode($res);
	}
	...
	//充值短信下订单
	public function doPageAddDxorder() {
		global $_W, $_GPC;
		$data['user_id'] = $_GPC['user_id'];
		$data['money'] = $_GPC['money'];
		...
		$data['code'] = time() . rand(1000, 9999) . $_GPC['user_id'];
		$data['time'] = date("Y-m-d H:i:s");
		$res = pdo_insert('cjdc_dxorder', $data);
		$order_id = pdo_insertid();
		if ($res) {
			echo $order_id;
		} else {
	...
	public function doPageAddHyOrder() {
		...
		if ($res) {
			...
			if ($_GPC['pay_type'] == 2 || $_GPC['money'] == 0) {
				$yxhy['uniacid'] = 7;
				$yxhy['openid'] = 'sns_wa_'.$user['openid'];
				$yxhy['member_card_id'] = 2;
				$yxhy['name'] = '优选会员卡';
				$yxhy['receive_time'] = time();
				$yxhy['expire_time'] = strtotime(date('Y-m-d',strtotime('+'. $_GPC['month'] . " month")));
				...
				pdo_insert('ewei_shop_member_card_history',$yxhy);
			}
			echo $orderid;
		} else {
			echo json_encode('下单失败');
	...
	public function doPageNewDsd(){
	 global $_W, $_GPC;
	 $store_id = $_GPC['store_id'];
	 $rese = pdo_getall('cjdc_order', array('state' => 3, 'store_id' => $store_id, 'type' => 1)); //待送达外卖
	 echo json_encode($rese);
	}
	
	public function doPageNewDsdtx(){
	 global $_W, $_GPC;
	 $store_id = $_GPC['store_id'];
	 $res = pdo_get('cjdc_order', array('state' => 3, 'store_id' => $store_id, 'type' => 1)); //待送达外卖
	 if ($res) {
			echo 1;
		}
	}
	...
	public function doPageNewOrder() {
		...
		$rese = pdo_get('cjdc_order', array('state' => 3, 'store_id' => $store_id, 'type' => 1)); //待送达外卖
	...
	public function doPageNewOrderMessage() {
		...
		function set_msg($_W) {
			...
			$res = pdo_get('cjdc_message', array('uniacid' => $_W['uniacid']));
			$res2 = pdo_get('cjdc_order', array('id' => $_GET['order_id']));
			$res3 = pdo_get('cjdc_order_goods',array('order_id'=>$_GPC['order_id']));
			$store = pdo_get('cjdc_store', array('id' => $res2['store_id']));
			if ($res2['order_type'] == 1) {
				$ordertype = '外卖配送';
			} elseif ($res2['order_type'] == 2) {
            ...
            'page'=>"zh_cjdianc/pages/Liar/loginindex",
            'data'=>[
                'thing2'=>array('value'=>$res3['name'].' '.$res3['spec']), 
                'amount1'=>array('value'=>$res2['money']), 
                'thing4'=>array('value'=>$res2['name']. ' ' .$res2['tel']), 
                'thing3'=>array('value'=>$res2['address']), 
				'date5'=>array('value'=>$res2['time']), 
					]
				];
			$url = "https://api.weixin.qq.com/cgi-bin/message/subscribe/send?access_token=" . $access_token . "";
			$formworks = json_encode($formwork);
...
//退款成功
	...
	public function doPageStoreTx() {
		...
		$store_id = $_GPC['store_id'];
		$sql="select b.poundage,b.tg_poundage from".tablename('cjdc_store')."a  left join ".tablename('cjdc_storetype')." b on a.md_type=b.id where a.id={$store_id}";
        $list=pdo_fetch($sql);
		if ($total_money >= $_GPC['tx_cost']) {
			$poundage = $_GPC['tx_cost']*(($list['tg_poundage']+$list['poundage'])/100);
	...
	public function doPageTxPoundage(){
		global $_W, $_GPC;
		$storeid = $_GPC['store_id'];
		$sql="select b.poundage,b.tg_poundage from".tablename('cjdc_store')."a  left join ".tablename('cjdc_storetype')." b on a.md_type=b.id where a.id={$storeid}";
        $list=pdo_fetch($sql);
		$data = $list['tg_poundage']+$list['poundage'];
		echo json_encode($data);
	}
	...
	public function doPageKtx() {
		...
		//获取商家手续费
		$sql = "select b.poundage,b.dn_poundage,b.dm_poundage,b.yd_poundage,b.tg_poundage,b.tg_dnpoundage,tg_dmpoundage,b.tg_ydpoundage from" . tablename('cjdc_store') . "a  left join " . tablename('cjdc_storetype') . " b on a.md_type=b.id where a.id={$storeid}";
		$list4 = pdo_fetch($sql);
		...
		$sql2 = "select sum(money) as 'total_money',sum(ps_money) as ps_money,sum(ptps_money) as ptps_money,sum(yhq_money2) as hb_money,sum(commission) as commission,sum(yycommission) as yycommission,sum(fwycommission) as fwycommission from" . tablename("cjdc_order") . " as a" . $where;
		$list2 = pdo_fetch($sql2, $data);
				
		$dnwmcost = pdo_get('cjdc_order', array('store_id' => $storeid, 'dn_state ' => 2, 'pay_type' => array(1, 2), 'type' => 2), array('sum(money) as total_money', 'sum(yhq_money2) as hb_money','sum(commission) as commission','sum(yycommission) as yycommission','sum(fwycommission) as fwycommission'));
		...
		$commission = $list2['commission']+$list2['yycommission']+$list2['fwycommission']+$dnwmcost['commission']+$dnwmcost['yycommission']+$dnwmcost['fwycommission']+$dmcost['commission']+$dmcost['yycommission']+$dmcost['fwycommission']+$yycost['commission']+$yycost['yycommission']+$yycost['fwycommission'];

		$ktxcost=number_format(($list2['total_money']+$dnwmcost['total_money']+$dmcost['total_money']+$yycost['total_money'])-$commission-$total['tx_cost']-$total2['tx_cost']-$ps_money-$list2['ptps_money']+$tuan,2);
		//$ktxcost = number_format(($list2['total_money']+$list2['hb_money']+$dnwmcost['total_money']+$dnwmcost['hb_money']+$dmcost['total_money']+$yycost['total_money'])-((($list2['total_money']+$list2['hb_money']-$list2['ps_money'])*($list4['poundage']+$list4['tg_poundage'])+($dnwmcost['total_money']+$dnwmcost['hb_money'])*($list4['dn_poundage']+$list4['tg_dnpoundage'])+$dmcost['total_money']*($list4['dm_poundage']+$list4['tg_dmpoundage'])+$yycost['total_money']*($list4['yd_poundage']+$list4['tg_ydpoundage']))/100)-$total['tx_cost']-$total2['tx_cost']-$ps_money+$tuan, 2, '.', '');
		return $ktxcost;
	}
...
//已付款
	...
	public function doPageAddCouponsApi() {
		global $_W, $_GPC;
		$data['name'] = $_GPC['name']; //优惠券名称
		$data['category'] = $_GPC['category'];
		$data['commission'] = $_GPC['commission'];
		$data['commission_td'] = $_GPC['commission_td'];
		$data['lq_num'] = $_GPC['lq_num'];
		$data['full'] = $_GPC['full']; //满
		$data['reduce'] = $_GPC['reduce']; //减
		$data['number'] = $_GPC['number']; //数量
		$data['start_time'] = $_GPC['start_time']; //开始时间
		$data['end_time'] = $_GPC['end_time']; //结束时间
		$data['uniacid'] = $_W['uniacid'];
		$data['type'] = $_GPC['type']; //1外卖2店内3通用
		$data['store_id'] = $_GPC['store_id']; //店铺id
		$data['instruction'] = $_GPC['instruction']; //说明
		if($_GPC['id']==0){
	...
	public function doPageCouponDs(){
		global $_W, $_GPC;
		$res = pdo_get('cjdc_coupons',array('id'=>$_GPC['id']));
		echo json_encode($res);
	}
	...
	public function doPageMyCouponsApi() {
		global $_W, $_GPC;
		$pageindex = max(1, intval($_GPC['page']));
		$sql = "select  id,name,reduce,start_time,end_time,number,stock,full,commission,commission_td,instruction,store_id,category from " . tablename("cjdc_coupons") . " where store_id={$_GPC['store_id']} and category!=3 order by id DESC";
		$list = pdo_fetchall($sql);	
		echo json_encode($list);
	...
	public function doPageDelCouponsApi() {
		global $_W, $_GPC;
		$res = pdo_delete('cjdc_coupons', array('id' => $_GPC['id']));
		if ($res) {
			$sql = "select  id,name,reduce,start_time,end_time,number,stock,full,commission,commission_td,instruction,store_id,category from " . tablename("cjdc_coupons") . " where store_id={$_GPC['store_id']} and category!=3 order by id DESC";
		    $list = pdo_fetchall($sql);
			echo json_encode($list);
	...
	//查询分享领取优惠券
	public function doPageCouponCx(){
		global $_W, $_GPC;
		$uid = $_GPC['uid'];
		$user_id = $_GPC['user_id'];
		$time = $_GPC['time'];
		$cc = pdo_get('cjdc_usercoupons',array('id'=>$_GPC['coupon_id']));
		$data = pdo_get('cjdc_usercoupons',array('uid'=>$uid,'user_id'=>$user_id,'fxtime'=>$time,'coupon_id'=>$cc['coupon_id']));
		$sql = "select  a.*,b.name as store_name from " . tablename("cjdc_coupons") . " a" . " left join " . tablename("cjdc_store") . " b on b.id=a.store_id where a.id=" . $cc['coupon_id'];
	...
	public function doPageYyLq(){
		global $_W, $_GPC;
		$uid = $_GPC['uid'];
		$user_id = $_GPC['user_id'];
		$coupon_id = $_GPC['coupon_id'];
		$coupon = pdo_get('cjdc_usercoupons',array('id'=>$coupon_id));
		if($coupon['couponnum']=='-1'&&$coupon['state']==2){
		 ...
		 }else if($coupon['couponnum']==0&&$coupon['state']==2){
			if($coupon['uid']==0){
			$res = pdo_update('cjdc_usercoupons',array('uid'=>$uid,'user_id'=>$user_id,'time'=>date('Y-m-d H:i:s')),array('id'=>$coupon_id));
			}else{
			$res = pdo_update('cjdc_usercoupons',array('user_id'=>$user_id,'time'=>date('Y-m-d H:i:s')),array('id'=>$coupon_id));
			}
			if($res){
				echo '1';
			}else{
		 ...
		 }else if($coupon['couponnum']>1&&$coupon['state']==2){
			...
			$data['type'] = 1; //手动领取
			$res = pdo_insert('cjdc_usercoupons', $data);
				 pdo_update('cjdc_usercoupons',array('couponnum -='=>1),array('id'=>$coupon_id));	
			...
			if($res){
			 ...
			 if($coupon['uid']==0){
			$res = pdo_update('cjdc_usercoupons',array('uid'=>$uid,'user_id'=>$user_id,'couponnum -='=>1,'time'=>date('Y-m-d H:i:s')),array('id'=>$coupon_id));	
			 }else{
			$res = pdo_update('cjdc_usercoupons',array('user_id'=>$user_id,'couponnum -='=>1,'time'=>date('Y-m-d H:i:s')),array('id'=>$coupon_id));					 
			 }
			if($res){
				echo '1';
			}else{
			...
			echo json_encode('优惠券不存在或已被使用！');
		 }
	}
	
	//分享优惠券
	public function doPageCouponFx(){
		global $_W, $_GPC;
		$uid = $_GPC['uid'];
		$user_id = $_GPC['user_id'];
		$coupon_id = $_GPC['coupon_id'];
		$time = $_GPC['time'];
	 $coupon = pdo_get('cjdc_usercoupons',array('id'=>$coupon_id));
	 $usercoupon = pdo_get('cjdc_usercoupons',array('fxtime'=>$time,'uid'=>$uid,'user_id'=>$user_id,'coupon_id'=>$coupon['coupon_id']));
	 if(!$usercoupon){
		 if($coupon['couponnum']=='-1'&&$coupon['state']==2){
			$data['user_id'] = $user_id;
			$data['uid'] = $uid;
			...
			if($coupon['uid']==0){
			$res = pdo_update('cjdc_usercoupons',array('uid'=>$uid,'user_id'=>$user_id,'fxtime'=>$time,'time'=>date('Y-m-d H:i:s')),array('id'=>$coupon_id));
			}else{
			$res = pdo_update('cjdc_usercoupons',array('user_id'=>$user_id,'fxtime'=>$time,'time'=>date('Y-m-d H:i:s')),array('id'=>$coupon_id));
			}
			if($res){
				echo '1';
			}else{
			...
			if($res){
			 ...
			 if($coupon['uid']==0){
			$res = pdo_update('cjdc_usercoupons',array('uid'=>$uid,'user_id'=>$user_id,'couponnum -='=>1,'fxtime'=>$time,'time'=>date('Y-m-d H:i:s')),array('id'=>$coupon_id));	
			 }else{
			$res = pdo_update('cjdc_usercoupons',array('user_id'=>$user_id,'couponnum -='=>1,'fxtime'=>$time,'time'=>date('Y-m-d H:i:s')),array('id'=>$coupon_id));					 
			 }
			if($res){
				echo '1';
			}else{
...
//喜讯
	...
	public function doPageSaveTel() {
		global $_GPC, $_W;
		$res = pdo_update('cjdc_user', array('tel' => $_GPC['tel']), array('id' => $_GPC['user_id']));
		if ($res) {
			echo '1';
		} else {
			echo '2';
