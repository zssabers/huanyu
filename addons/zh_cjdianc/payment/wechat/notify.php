...
function addLog($filename, $string)
{
	...
	$dxorder=pdo_get('cjdc_dxorder',array('code'=>$logno));
	...
	if($dxorder and $dxorder['state']==1){
		 pdo_update('cjdc_dxorder',array('state'=>2),array('id'=>$dxorder['id']));
		 pdo_update('cjdc_store', array('note_num +=' => $dxorder['money2']), array('id' => $dxorder['storeid']));
	}
    if($czorder and $czorder['state']==1){
        pdo_update('cjdc_czorder',array('state'=>2),array('id'=>$czorder['id']));
		$vip = pdo_get('cjdc_storevip',array('user_id'=>$czorder['user_id'],'store_id'=>$czorder['storeid']));
		if($czorder['type']==1){
			 ...
			 $coupons['user_id'] = $czorder['user_id'];
			 $coupons['coupon_id'] = $czorder['couponid'];
			 $coupons['state'] =2;
			 $coupons['uniacid'] = $czorder['uniacid'];
			 $coupons['time'] = date("Y-m-d H:i:s");
			 $coupons['type'] = 2;
			 $coupons['couponnum'] = $czorder['couponnum'];
			 
			if($vip){
			 $money = $vip['money']+$czorder['money'];
			 
			 pdo_update('cjdc_storevip',array('money' =>$money),array('user_id'=>$czorder['user_id'],'store_id'=>$czorder['storeid']));	
			 pdo_insert('cjdc_usercoupons',$coupons);
			}else{
			 $storevip['user_id'] = $czorder['user_id'];
			 $storevip['store_id'] = $czorder['storeid'];
			 $storevip['money'] = $czorder['money'];
			 $storevip['cater_time'] = date("Y-m-d");
			 $storevip['uniacid'] = $czorder['uniacid'];
			 
			 pdo_insert('cjdc_usercoupons',$coupons);
			 pdo_insert('cjdc_storevip',$storevip);
			}
		}else{
    ...
    if($order['type']==1 and ($order['state']==1 || $order['state']==6)){//外卖
        ...
        //模板消息
		if(($store['is_jd'] == 1 || $store['print_mode'] == 1)){
