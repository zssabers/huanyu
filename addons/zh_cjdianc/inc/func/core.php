		...
		if ($_W['role'] == 'operator') {
			...
			$navemenu[0] = array(
				...
				'items' => array(
					...
					7 => $this->createMainMenu('短信审核', $do, 'note', ''),
				),
			);
			$navemenu[1] = array(
			...
			if ($system['hygn'] == 1) {
				$navemenu[16] = array(
					...
					'items' => array(
						...
						4 => $this->createMainMenu('特邀优惠券', $do, 'hycoupons', ''),
					),
				);
			}
			//$navemenu[17] = array(
			...
			$navemenu[13] = array(
				...
				'items' => array(
					...
					5 => $this->createMainMenu('充值活动', $do, 'hdsz', ''),
					6 => $this->createMainMenu('活动商家', $do, 'hdsj', ''),
					7 => $this->createMainMenu('短信套餐', $do, 'dxcz', ''),
				),
			);
			// $navemenu[15] = array(
		...
		if ($_W['isfounder'] || $_W['role'] == 'manager' || $_W['role'] == 'operator') {
			$navemenu[0] = array(
				...
				'items' => array(
					...
					//6 => $this->createMainMenu('员工管理 ', $do, 'test2', ''),
			...
			$navemenu[4] = array(
				...
				'items' => array(
					...
					// 4 => $this->createMainMenu('积分设置 ', $do, 'injfset', ''),
					// 5 => $this->createMainMenu('短信通道 ', $do, 'insms', ''),
		...
		if ($account['role'] == 1) {
			$navemenu[0] = array(
				...
				'items' => array(
					...
					8 => $this->createSubMenu('员工管理 ', $do, 'test2','fa-angle-right', $cur_color, $storeid),
				),
				'icon' => 'fa fa-user-md',
			);
			$cur_color = '#8d8d8d';
			...
			$navemenu[14] = array(
				...
				'title' => '<a href="cjdianc.php?c=site&a=entry&op=display&do=dlaccount&m=zh_cjdianc" class="panel-title wytitle" id="yframe-14"><icon style="color:#8d8d8d;" class="fa fa-newspaper-o"></icon>  子账号管理</a>',
				'items' => array(

					1 => $this->createSubMenu('子账号管理 ', $do, 'dlaccount', 'fa-angle-right', $cur_color, $storeid),
					2 => $this->createSubMenu('子账号添加 ', $do, 'dladdaccount', 'fa-angle-right', $cur_color, $storeid),
				),
			);
...
//改变佣金
	...
	public function QxDada($order_id) {
		global $_W, $_GPC;
		include IA_ROOT . '/addons/zh_cjdianc/peisong/peisong.php';
		$order = pdo_get('cjdc_order', array('id' => $order_id));
		$set = pdo_get('cjdc_psset', array('store_id' => $order['store_id']));
...
//喜讯
	...
	public function storePoundage($storeid) {
		$sql = "select b.dm_poundage,b.dn_poundage,b.poundage,b.yd_poundage,b.tg_dmpoundage,b.tg_dnpoundage,b.tg_poundage,b.tg_ydpoundage from" . tablename('cjdc_store') . "a  left join " . tablename('cjdc_storetype') . " b on a.md_type=b.id where a.id={$storeid}";
		$list = pdo_fetch($sql);
