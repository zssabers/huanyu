...
$sql="select a.* ,b.img,b.name,b.tgy,b.store_num from " . tablename("cjdc_retail") . " a"  . " left join " . tablename("cjdc_user") . " b on b.id=a.user_id  ". $where." ORDER BY id DESC";
...
foreach ($list as $key => $value) {
  ...
  $storenum=pdo_get('cjdc_store', array('user_uid'=>$value['user_id']), array('count(id) as count'));
 if($sys['is_ej']==2){
...
if($storenum['count']){
  $list[$key]['storenum']=$storenum['count'];
}
if($yj['djyj']){
...
if($operation=='adopt'){//审核通过
    $id=$_GPC['id'];
	$tgy = pdo_get('cjdc_retail',array('id'=>$id));
    $res=pdo_update('cjdc_retail',array('state'=>2,'sh_time'=>time()),array('id'=>$id));
	  	
    if($res){
		pdo_update('cjdc_user',array('tgy'=>1,'store_num'=>30),array('id'=>$tgy['user_id']));
        message('审核成功',$this->createWebUrl('fxlist',array()),'success');
    }else{
        message('审核失败','','error');
    }
}
if($operation=='tgy'){//升级通过
    $id=$_GPC['id'];
	$tgy = pdo_get('cjdc_retail',array('id'=>$id));
    $res=pdo_update('cjdc_user',array('tgy'=>1,'store_num'=>30),array('id'=>$tgy['user_id']));  
    if($res){
        message('升级成功',$this->createWebUrl('fxlist',array()),'success');
    }else{
        message('升级失败','','error');
    }
}
...
if(checksubmit('submit')){
 ...
 if(checksubmit('submits')){
  $data2['store_num']=$_GPC['store_num'];
   $res=pdo_update('cjdc_user',array('store_num'=>$_GPC['store_num']),array('id'=>$_GPC['user_id']));
  if($res){
      message('更改成功',$this->createWebUrl('fxlist',array()),'success');
  }else{
       message('更改成功','','error');
  }
  }
