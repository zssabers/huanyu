...
if($_GPC['keywords']){
  ...
  $inuser=join(",",array_column2($inuser,'user_id'));
	...
	$sql = "select * from " . tablename("cjdc_coupons") . "WHERE  UNIX_TIMESTAMP(start_time)<={$time} and  UNIX_TIMESTAMP(end_time)>={$time} and store_id={$storeid} and category=1";
...
if(checksubmit('zengsong')){
      if($_GPC['coupon']!=''){
       ...
       if($res){
       message('赠送成功！', $this->createWebUrl('inuser'), 'success');
      }else{
       message('赠送失败！','','error');
      }
