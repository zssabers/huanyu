...
$sql31="select sum(money) as 'total_money',sum(yhq_money2) as hb_money,sum(commission) as commission,sum(yycommission) as yycommission,sum(fwycommission) as fwycommission from" . tablename("cjdc_order") ." as a".$where3." and pay_type=6";
...
//店内线下支付总金额
$dnxxcost=pdo_get('cjdc_order', array('store_id'=>$storeid,'dn_state '=>2,'pay_type'=>5,'type'=>2), array('sum(money) as total_money','sum(yhq_money2) as hb_money','sum(commission) as commission','sum(yycommission) as yycommission','sum(fwycommission) as fwycommission'));
//店内余额支付总金额
