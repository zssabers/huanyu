...
if(checksubmit('submit')){
	if($_GPC['category']==2){
		if($_GPC['commissiontype']==1){
			if($_GPC['commission']==0){
				message('异业优惠券固定佣金不能为0元','','error');
			}
		}else if($_GPC['commissiontype']==2){
			if($_GPC['commission_td']<1){
				message('异业优惠券佣金提点不能低于1%','','error');
			}
		}
	}	
		$data['name']=$_GPC['name'];
		...
		$data['is_dj']=$_GPC['is_dj'];
		$data['is_lqs']=$_GPC['is_lqs'];
		$data['dc_num']=$_GPC['dc_num'];
	if($_GPC['id']==''){
