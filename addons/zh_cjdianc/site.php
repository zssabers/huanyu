...
class zh_cjdiancModuleSite extends Core {
	...
	public function doMobileNewDsd(){
	 global $_W, $_GPC;
	 $store_id = $_GPC['store'];
	 $count = pdo_get('cjdc_order', array('store_id' => $store_id,'state'=>3), array('count(id) as count'));
	 echo json_encode($count['count']);
	}
	
	public function doMobileNewOrders() {
		global $_W, $_GPC;
		$store_id = $_GPC['store'];
	...
	//删除短信套餐
	public function doMobileDelDx() {
		global $_W, $_GPC;
		$res = pdo_delete('cjdc_storenote', array('id' => $_GPC['id']));
		if ($res) {
			echo '1';
		} else {
			echo '2';
	...
	//添加短信套餐
	public function doMobileAddDx() {
		global $_W, $_GPC;
		for ($i = 0; $i < count($_GPC['list']); $i++) {
			$data['money'] = $_GPC['list'][$i]['money'];
			$data['num'] = $_GPC['list'][$i]['num'];
			$data['des'] = $_GPC['list'][$i]['des'];
			$data['uniacid'] = $_W['uniacid'];
			pdo_insert('cjdc_storenote', $data);
		}
	}
	...
	public function doMobileZsq() {
	      global $_W, $_GPC;
		  $id = explode(',',$_GPC['id']);
		  $coupon = pdo_get('cjdc_coupons',array('id'=>$_GPC['coupon']));
	...
	public function doMobileZh() {
		global $_W, $_GPC;
		$data['user_id'] = $_GPC['id'];
		$data['store_id'] = $_GPC['storeid'];
		$data['document'] = $_GPC['document'];
		$data['sq_time'] = time();
		$data['uniacid'] = $_W['uniacid'];
		$id = explode(',',$data['user_id']);
		$ids = count($id);
		
		$store = pdo_get('cjdc_store',array('id'=>$_GPC['storeid']));
		if($store['note_num']>=$ids){
		$res = pdo_insert('cjdc_userzh',$data);
		}else{
			message('剩余短信不足,请去小程序内充值!','','error');
		}
		if ($res) {
			pdo_update('cjdc_store',array('note_num -='=>$ids),array('id'=>$_GPC['storeid']));
			echo '1';
		} else {
			echo '2';
		}
...
//获取下级店铺
	public function doMobileGetDp() {
		global $_W, $_GPC;
		$user_id = $_GPC['user_id'];
		$sql = " select a.name as storename,b.name as fx_name,a.sq_time from" . tablename('cjdc_store') . " a left join " . tablename('cjdc_user') . " b on a.user_uid=b.id where a.user_uid={$user_id}";
...
////下架
	...
	public function  doMobileYyCoade(){
		   global $_W, $_GPC;
			...
			function set_msg2($userid,$storeid){
				$access_token = getaccess_token2();
				$data2=array(
					"scene"=>$userid.'-'.$storeid,
			...
			$img=set_msg2($_GPC['userid'],$_GPC['storeid']);
