...
class Index_EweiShopV2Page extends AppMobilePage
{
    ...
    public function getlist()
    {
		...
		$usercoupons = pdo_getall('ewei_shop_coupon_data', array('openid' => $_W['openid']));
		
			for ($i = 0; $i < count($coupons); $i++) {
					$coupons[$i]['state'] = 2;
			 for ($k = 0; $k < count($usercoupons); $k++) {
				if ($coupons[$i]['id'] == $usercoupons[$k]['couponid']) {
        ...
        foreach( $coupons as $i => &$row ) 
        {
            ...
            $newCoupon = array( "state"=>$row['state'] ,"id" => $row["id"], "isdisa" => $row["isdisa"], "color" => trim($row["color"]), "thumb" => $row["thumb"], "couponname" => $row["couponname"], "tagtitle" => $row["tagtitle"], "backtype" => $row["backtype"], "deduct" => $row["deduct"], "discount" => (double) $row["discount"], "backmoney" => $row["backmoney"], "backcredit" => $row["backcredit"], "backredpack" => $row["backredpack"], "timestr" => $row["timestr"], "gettypestr" => $row["gettypestr"], "lastratio" => $row["lastratio"], "title2" => $title2, "iconurl" => ($row["isdisa"] ? $_W["siteroot"] . "template/mobile/default/static/images/coupon/end.png" : "") );
            if( $row["timestr"] == "0" ) 
            {
			...
			$newCoupon['starttime'] = date('Y-m-d',$row['timestart']);
			$newCoupon['endtime'] = date('Y-m-d',$row['timeend']);
			$newCoupon["full"]=$row['enough'];
			$newCoupon["reduce"]=$row['deduct'];
