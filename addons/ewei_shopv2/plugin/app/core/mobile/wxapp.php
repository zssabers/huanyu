	...
	public function auth() 
	{
		...
		if( $errCode == 0 ) 
		{
			...
			if( empty($member) ) 
			{
				...
				if($_GPC['mid']) {
					$p = m("member")->getMember($_GPC['mid']);
					if($p && $p['status'] == 1) {
						$member['agentid'] = $_GPC['mid'];
					}
				}
				$cjdc = pdo_get('cjdc_user',array('openid'=>$data['openId']));
				if(empty($cjdc)){
				$cjdata = array("uniacid"=>9,'join_time'=>time(),"name"=>$data['nickName'],"openid"=>$data['openId'],'img'=>$data['avatarUrl']);
				  pdo_insert('cjdc_user',$cjdata);	
				  $fxid = pdo_insertid();
				  if($_GPC['mid']){
					$yxuser = pdo_get('ewei_shop_member',array('id'=>$_GPC['mid']));
					$cjopenid = str_replace('sns_wa_','',$yxuser['openid']);
					$cjdcs = pdo_get('cjdc_user',array('openid'=>$cjopenid));
					$cjdc = $cjdcs['id'];
				  }else{
					$cjid=0;  
				  }
				  //pdo_insert('cjdc_fxuser',array('fx_user'=>$fxid,'user_id'=>$cjid,'time'=>date('Y-m-d H:i:s', time())));
				  //pdo_insert('cjdc_retail',array('user_id'=>$cjid,'time'=>time(),'sh_time'=>time(),'uniacid'=>9,'user_name'=>$data['nickName'],'state'=>2));
				}
