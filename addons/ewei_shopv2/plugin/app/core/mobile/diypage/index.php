		...
		if (!(empty($dataurl))) 
		{
			if (strpos($dataurl, '/pages/') === false) 
			{
				...
				else if ($dataParams[0] == 'groups') 
				{
					...
					$goodsql = 'SELECT id,title,subtitle,thumb,hascommission,nocommission,commission,commission1_rate,commission1_rate,marketprice,commission1_pay,maxprice,marketprice,productprice,minprice,maxprice,isdiscount,isdiscount_time,isdiscount_discounts,sales,salesreal,total,description,bargain,`type`,ispresell,`virtual`,hasoption,video FROM ' . tablename('ewei_shop_goods') . ' WHERE id in(' . $goodsid . ') AND status > 0 AND deleted = 0 AND uniacid =7 ORDER BY displayorder DESC limit 0,' . $goodsnum;
				...
				else if ($dataParams[0] == 'goodsids') 
				{
					...
					if (!(empty($goodsids))) 
					{
						...
						$sql = 'SELECT id,title,subtitle,thumb,marketprice,hascommission,nocommission,commission,commission1_rate,commission1_rate,marketprice,commission1_pay,maxprice,productprice,minprice,maxprice,isdiscount,isdiscount_time,isdiscount_discounts,sales,salesreal,total,description,bargain,`type`,ispresell,`virtual`,hasoption,video FROM ' . tablename('ewei_shop_goods') . ' WHERE id in(' . $goodsid . ') AND uniacid =7 ORDER BY displayorder DESC limit 0,' . $goodsnum;
						$count = pdo_fetch('SELECT count(id) as count FROM ' . tablename('ewei_shop_goods') . ' WHERE id in(' . $goodsid . ') AND uniacid =' . $_W['uniacid']);
