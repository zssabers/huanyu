...
class Create_EweiShopV2Page extends AppMobilePage 
{
	...
	public function submit() 
	{
		...
		$scorder = pdo_insert("ewei_shop_order", $order);
		$orderid = pdo_insertid();
		
		if($scorder){
			$openids = str_replace('sns_wa_','',$openid);
			$user_id = pdo_get('cjdc_user',array('openid'=>$openids));
			$commission = pdo_get('cjdc_fxset', array('uniacid' => 9));
			$user = pdo_get('cjdc_fxuser', array('fx_user' => $user_id['id']));
				if ($user['user_id']) {
					if ($order['type'] == 1) {
						$money = $totalprice * ($commission['wm_yj'] / 100); //一级佣金
					} else {
						$money = $totalprice * ($commission['dn_yj'] / 100); //一级佣金
					}
					$userid = $user['user_id']; //上线id
					$data6['user_id'] = $userid; //上线id
					$data6['son_id'] = $user_id['id']; //下线id
					$data6['money'] = $money; //金额
					$data6['time'] = time(); //时间
					$data6['order_id'] = $orderid;
					$data6['is_shop'] = 1;
					$data6['state'] = 1;
					$data6['note'] = '商城订单佣金';
					$data6['uniacid'] = 9;
