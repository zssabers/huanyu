									...
									if( !empty($goods) && is_array($goods) ) 
									{
										...
										foreach( $goodsids as $goodsid ) 
										{
											foreach( $goods as $index => $good ) 
											{
												if( $good["id"] == $goodsid ) 
												{
													if( empty($good["isdiscount"]) || $good["isdiscount_time"] < time() ) 
													{
														...
														$goods_price = pdo_fetch("select minprice,maxprice from " . tablename("ewei_shop_goods") . " where id=:id and uniacid=:uniacid limit 1", array( ":id" => $good["id"], ":uniacid" => $_W["uniacid"] ));
