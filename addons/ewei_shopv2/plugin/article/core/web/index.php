...
class Index_EweiShopV2Page extends PluginWebPage 
{
	...
	protected function post() 
	{
			...
			if (is_array($_GPC['thumbs'])) 
				{
					...
					$data['resp_img'] = save_media($thumb_url[0]);
					unset($thumb_url[0]);
		...
		$categorys = pdo_fetchall("SELECT * FROM " . tablename("ewei_shop_article_category") . " WHERE uniacid=:uniacid ", array( ":uniacid" => $_W["uniacid"] ));
		$goods = pdo_fetchall("SELECT id,title FROM " . tablename("ewei_shop_goods") . " WHERE uniacid=:uniacid ", array( ":uniacid" => $_W["uniacid"] ));
		$article = pdo_fetch("SELECT * FROM " . tablename("ewei_shop_article") . " WHERE id=:aid and uniacid=:uniacid limit 1 ", array( ":aid" => $aid, ":uniacid" => $_W["uniacid"] ));
		if( !empty($article) ) 
		{
	...
	{
		$piclist = array_merge(array($article['resp_img']), iunserializer($article['thumb_url']));
	}
		include($this->template());
	}
