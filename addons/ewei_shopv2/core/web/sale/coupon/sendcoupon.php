...
class Sendcoupon_EweiShopV2Page extends ComWebPage 
{
	...
	public function sendmessage() 
	{
		...
		else if ($messagetype == 1) 
		{
			...
			{
				exit(json_encode(array('result' => 0, 'message' => '未指设定发送模板!', 'openid' => $openid)));
			}
			if (empty($openid)) 
			{
				exit(json_encode(array('result' => 0, 'message' => '未指定openid!', 'openid' => $openid)));
			}
			...
			if (is_error($result)) 
			{
				exit(json_encode(array('result' => 0, 'message' =>'res1-'.$result['message'], 'openid' => $openid)));
			}
		...
		else if ($messagetype == 2) 
		{
			...
			if (is_error($resp)) 
			{
				exit(json_encode(array('result' => 0, 'message' =>'res2-'.$resp['message'], 'openid' => $openid)));
			}
			exit(json_encode(array('result' => 1)));
		}
		else 
