...
class Index_EweiShopV2Page extends WebPage {
    public function main($goodsfrom = 'sale') {
        ...
        if (empty($_W['shopversion'])) {
            ...
            if (empty($goodsfrom)) {
                header('location: ' . webUrl('goods', array(
                        'goodsfrom' => 'sale'
                    )));
            }
        ...
        if (!(empty($total))) {
            ...
            foreach ($list as $key => & $value) {
                ...
                $url = mobileUrl('goods/detail', array(
                    'id' => $value['id']
                ) , true);
    ...
    public function create() {
        ...
        if ($_W['ispost']) {
            ...
            if (is_array($_GPC['cates'])) {
                ...
                foreach ($cates as $key => $cid) {
                    $c = pdo_fetch('select level from ' . tablename('ewei_shop_category') . ' where id=:id and uniacid=:uniacid limit 1', array(
                        ':id' => $cid,
                        ':uniacid' => $_W['uniacid']
                    ));
                    if ($c['level'] == 1) {
                    ...
                    if ($key == 0) {
                        if ($c['level'] == 1) {
                            ...
                            $crow = pdo_fetch('select parentid from ' . tablename('ewei_shop_category') . ' where id=:id and uniacid=:uniacid limit 1', array(
                                ':id' => $cid,
                                ':uniacid' => $_W['uniacid']
                            ));
            ...
            if ($data['type'] == 4) {
                ...
                if (0 < $intervalfloor) {
                    ...
                    $intervalprices[] = array(
                        'intervalnum' => intval($_GPC['intervalnum1']) ,
                        'intervalprice' => floatval($_GPC['intervalprice1'])
                    );
                }
                if (1 < $intervalfloor) {
                    ...
                    $intervalprices[] = array(
                        'intervalnum' => intval($_GPC['intervalnum2']) ,
                        'intervalprice' => floatval($_GPC['intervalprice2'])
                    );
                }
                if (2 < $intervalfloor) {
                    ...
                    $intervalprices[] = array(
                        'intervalnum' => intval($_GPC['intervalnum3']) ,
                        'intervalprice' => floatval($_GPC['intervalprice3'])
                    );
                }
            ...
            $goodsinfo = pdo_fetch($sqlgoods, array(
                ':id' => $id,
                ':uniacid' => $_W['uniacid']
            ));
        ...
        $com_virtual = com('virtual');
        $dispatch_data = pdo_fetchall('select * from ' . tablename('ewei_shop_dispatch') . ' where uniacid=:uniacid and merchid=:merchid and enabled=1 order by displayorder desc', array(
            ':uniacid' => $_W['uniacid'],
            ':merchid' => $merchid
        ));
        ...
        if ($com_virtual) {
            $virtual_types = pdo_fetchall('select * from ' . tablename('ewei_shop_virtual_type') . ' where uniacid=:uniacid and merchid=:merchid and recycled = 0 order by id asc', array(
                ':uniacid' => $_W['uniacid'],
                ':merchid' => 0
            ));
        }
    ...
    protected function post() {
        require dirname(preg_replace('@\(.*\(.*$@', '', __FILE__)) . '/post.php';
    }
    ...
    public function delete1() {
        ...
        $items = pdo_fetchall('SELECT id,title FROM ' . tablename('ewei_shop_goods') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid']);
        foreach ($items as $item) {
            pdo_delete('ewei_shop_goods', array(
                'id' => $item['id']
            ));
    ...
    public function change() {
        ...
        $goods = pdo_fetch('select id,hasoption,marketprice,dowpayment from ' . tablename('ewei_shop_goods') . ' where id=:id and uniacid=:uniacid limit 1', array(
            ':uniacid' => $_W['uniacid'],
            ':id' => $id
        ));
        if (empty($goods)) {
        ...
        $result = pdo_update('ewei_shop_goods', array(
            $type => $value
        ) , array(
            'id' => $id
        ));
    ...
    public function ajax_batchcates() {
        ...
        foreach ($goodsids as $goodsid) {
            if (!(empty($iscover))) {
                ...
                $goods = pdo_fetch('select pcate,ccate,tcate,cates,pcates,ccates,tcates  from ' . tablename('ewei_shop_goods') . ' where id=:id and uniacid=:uniacid limit 1', array(
                    ':id' => $goodsid,
                    ':uniacid' => $_W['uniacid']
                ));
