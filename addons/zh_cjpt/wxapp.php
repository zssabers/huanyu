...
class Zh_cjptModuleWxapp extends WeModuleWxapp {
	...
	public function doPageAddOrder(){
		...
		$data['ps_num']=$ps_num;
		$psmoney=peisong::getMoney($_POST['sender_lat'],$_POST['sender_lng'],$_POST['receiver_lat'],$_POST['receiver_lng'],$_POST['uniacid']);
		//var_dump($_POST['uniacid']);
		//die;
		if($psmoney){
		$data['ps_money']=$psmoney;
		$data['state']=1;
		$data['time']=time();
		$data['uniacid']=$_POST['uniacid'];
		...
		file_get_contents("".$_W['siteroot']."app/index.php?i=".$_POST['uniacid']."&c=entry&a=wxapp&do=SendMail&m=zh_cjpt");exit();//发邮件
		}else{
...
//骑手短信
	public function doPageSmstest(){
		global $_W, $_GPC;
		$res=pdo_get('cjpt_system',array('uniacid'=>$_POST['uniacid']));
		$tpl_id=$res['tpl_id4'];			
		$tel='15172415261';
		//$code=urlencode($_GPC['code']);
		$key=$res['appkey'];
...
//完成
	public function doPageComplete(){
		...
		if($rst){
			//订单完成
			$orderInfo=pdo_get('cjpt_dispatch',array('id'=>$_GPC['order_id']));
			$orderid = pdo_get('cjdc_order',array('order_num'=>$orderInfo['order_id']));
			pdo_update('cjdc_order',array('state'=>4,'complete_time'=>date('Y-m-d H:i:s',time())),array('order_num'=>$orderInfo['order_id']));
			pdo_update('cjdc_earnings', array('state' => 2), array('order_id' => $orderid['id']));
			echo '1';
		}else{
			echo '完成失败';
		}
...
public function doPageqxOrder(){
	...
	$bind=pdo_get('cjpt_bind',array('pt_uniacid'=>$_W['uniacid']));
	if($bind['cy_uniacid']==$_POST['uniacid']){
		$info=pdo_get('cjpt_dispatch',array('order_id'=>$_POST['order_id'],'uniacid'=>$_POST['uniacid']));
		if($info['state']==1){
...
public function doPageGetOrderInfo(){
	...
	$bind=pdo_get('cjpt_bind',array('pt_uniacid'=>$_W['uniacid']));
	if($bind['cy_uniacid']==$_POST['uniacid']){
	$sql=" select a.*,b.name,b.tel from".tablename('cjpt_dispatch')." a left join ".tablename('cjpt_rider')."b on a.qs_id=b.id where a.order_id={$_POST['order_num']} and a.uniacid={$_POST['uniacid']}";
	$info=pdo_fetch($sql);
	if($info){
 ...
 public function doPageMessage(){
      ...
      //设置与发送模板信息
       function set_msg($_W){
         ...
         $res=pdo_get('cjpt_message',array('uniacid'=>$_POST['uniacid']));
...
//发送邮件
     public function doPageSendMail(){
     	global $_W, $_GPC;
     	$config=pdo_get('cjpt_mail',array('uniacid'=>$_POST['uniacid']));
     	if($config['is_email']==1){
     		$user_email=pdo_getall('cjpt_rider',array('uniacid'=>$_POST['uniacid'],'state'=>2,'status'=>1),'email');
