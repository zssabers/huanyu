...
<div class="page-header">
    当前位置：
	<span  class="text-primary"><?php  if(!empty($item['id'])) { ?>编辑<?php  } else { ?>添加<?php  } ?>广告 <small><?php  if(!empty($item['id'])) { ?>修改【<?php  echo $item['advname'];?>】<?php  } ?></small></span>
</div>
    ...
    <div class="page-sub-toolbar">
        <span class=''>
            <?php if(cv('app.shop.banner.add')) { ?>
                <a class="btn btn-primary btn-sm" href="<?php  echo webUrl('article/articleadv/add')?>">添加新广告</a>
            <?php  } ?>
        </span>
    </div>
    <form <?php if( ce('app.shop.banner' ,$item) ) { ?>action="" method="post"<?php  } ?> class="form-horizontal form-validate" enctype="multipart/form-data">
        <input type="hidden" name="id" value="<?php  echo $item['id'];?>" />
 
                <div class="form-group">
                    <label class="col-lg control-label">排序</label>
                    <div class="col-sm-9 col-xs-12">
                        <?php if( ce('app.shop.banner' ,$item) ) { ?>
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-9 col-xs-12">
                    	<?php if( ce('app.shop.banner' ,$item) ) { ?>
	                        ...
	                        <span class='help-block'>建议尺寸:702 * 350 , 请将所有广告图片尺寸保持一致</span>
                        <?php  } else { ?>
                    ...
                    <div class="col-sm-9 col-xs-12">
                    	<?php if( ce('app.shop.banner' ,$item) ) { ?>
	                        <div class="input-group form-group" style="margin: 0;">
								...
								<span data-input="#bannerlink" data-toggle="selectUrl" data-platform="wxapp" class="input-group-addon btn btn-default">选择链接</span>
							</div>
                        <?php  } else { ?>
            	...
            	<div class="form-group">
                    ...
                    <div class="col-sm-9 col-xs-12">
                           ...
                           <?php  } ?>
                       <input type="button" name="back" onclick='history.back()' <?php if(cv('app.shop.banner.add|app.shop.banner.edit')) { ?>style='margin-left:10px;'<?php  } ?> value="返回列表" class="btn btn-default" />
                    </div>
            	</div>
