...
<div class="form-group">
	...
	<div class="col-sm-9 col-xs-12">
		<select class="form-control select2" name="goods_id" data-rule-required='true'>
                                    <option value="0">请选择商品</option>
                                    <?php  if(is_array($goods)) { foreach($goods as $vo) { ?>
                                        <option value="<?php  echo $vo['id'];?>" <?php  if($article['goods_id']==$vo['id']) { ?> selected="selected"<?php  } ?>><?php  echo $vo['title'];?></option>
                                    <?php  } } ?>
                    </select>
	</div>
</div>
...
<div class="form-group">
 ...
 <!--E-F-W-W-W修复-->
 <div class="form-group">
	<label class="col-lg control-label">口令</label>
	<div class="col-sm-9 col-xs-12">
		<textarea class="form-control" rows="5" name="kouling"><?php  echo $article['kouling'];?></textarea>
	</div>
</div>
     ...
     <label class="col-sm control-label must">商品图片</label>
         <div class="col-md-9 gimgs">
              <?php  echo tpl_form_field_multi_image('thumbs',$piclist)?>
			   <span class="help-block image-block">您可以拖动图片改变其显示顺序，建议为正方型图片，第一张为缩略图尺寸建议宽度150，其他为详情页面图片尺寸建议宽度为320，并保持图片大小一致</span>
        </div>
    </div>
<script>
